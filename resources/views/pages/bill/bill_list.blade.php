...
@section('content')
    <h1 class="text-center text-success " >Managment of Bills</h1>
    <br>
    <button class="create" onclick="window.location.href = '{{ url('/Bill_form') }}'">
        <span>
            ...
            @foreach ($bill as $item)
                <tr>
                    ...
                    <td> 
                        ...
                        <br>
                        <br>
                        <form class="disableBc" action="{{ route('deleteBill', $item->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit">
                                <span class="transition"></span>
                ...
                <div class="custom-modal" id="exampleModal{{ $item->id }}">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('updateBill', $item->id) }}" method="POST">
                            @csrf
                            @method('PUT')
